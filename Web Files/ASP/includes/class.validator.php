...
//	************************************************************
	...
	function is_sane_dir ($path)
	{
		...
		if (strpos('.tmp', $path) !== FALSE)	 //Check tmp file for read/write capabilities
		{
            $f = @fopen($path, 'w+');
			if (!$f)
			{
...
//	Strip numbers from a string
	...
	{
		$Stripped = preg_replace("([0-9]+)","",$text);
		return ($Stripped);
	}
...
//	Strips letters from a string
	...
	{
		$Stripped = preg_replace("([A-Z]+)","",$text);
		return $Stripped;
	}
...
//	validity as a reachable internet host
	...
	function is_hostname ($hostname = "")
	{
		...
		$Bad = preg_replace("[-A-Z0-9\.]","",$hostname);
